...
pub fn train_classification(
    ...
    if examples.len() < 100000 {
        return Err(format!(
            "Insufficient classification examples: {} (need 100K+)",
            examples.len()
        ));
    }
    ...
    let optimal_weights =
        optimize_feature_weights(val_examples, &intent_centroids, &tone_centroids, config)?;

    // Phase 5: Calibrate confidence
    let calibration_map = calibrate_confidence(
        val_examples,
        &intent_centroids,
        &tone_centroids,
        &optimal_weights,
    ...
    let (accuracy, avg_confidence) =
        validate_classification_with_calculator(memory, val_examples, &intent_centroids, config)?;
...
fn validate_classification_with_calculator(
    ...
    for example in val_examples.iter().take(sample_size) {
        ...
        let result =
            calculator.calculate(&example.question, &mem, &spatial, &config.classification);
...
pub fn train_reasoning(
    ...
    if examples.len() < 49000 {
        return Err(format!(
            "Insufficient reasoning examples: {} (need 49K+)",
            examples.len()
        ));
    }
    ...
    // Phase 2: Ingest Q&A pairs using MemoryStore's ingest_hierarchy directly
    {
        ...
        for (idx, example) in train_examples.iter().enumerate() {
            if idx % 10 == 0 {
                ...
                let q_packet =
                    input::ingest_raw_with_config(&example.question, true, &config.classification);
                ...
                mem.ingest_hierarchy(
                    &q_hierarchy,
                    SourceKind::TrainingDocument,
                    &format!("reasoning_q:{}", idx),
                );

                // Build and ingest answer
                let a_packet =
                    input::ingest_raw_with_config(&example.answer, true, &config.classification);
    ...
    store_reasoning_model(
        memory,
        &optimal_scoring_weights,
    ...
    let (accuracy, avg_confidence, units_count) =
        validate_reasoning_with_scorer(memory, val_examples, &optimal_scoring_weights, config)?;
...
fn validate_reasoning_with_scorer(
    ...
    let avg_confidence = if valid_examples > 0 {
        total_score / valid_examples as f32
    } else {
        0.3
    };
...
pub fn train_predictive(
    ...
    if examples.len() < 200000 {
        return Err(format!(
            "Insufficient predictive examples: {} (need 200K+)",
            examples.len()
        ));
    }
    ...
    let (accuracy, avg_confidence) =
        validate_predictive_with_resolver(memory, &examples[..1000], config)?;
...
pub async fn train_full_pipeline(
    ...
    let total_units =
        class_metrics.units_created + reason_metrics.units_created + predict_metrics.units_created;
...
async fn validate_with_engine(engine: &crate::engine::Engine) -> Result<(f32, f32), String> {
    let test_cases = vec![
        (
            ...
            vec!["Paris", "capital", "France"],
        ),
        (
            "Explain photosynthesis",
            vec!["light", "energy", "plants", "process"],
        ),
        (
            "Compare Python and JavaScript",
            vec!["Python", "JavaScript", "language"],
        ),
...
fn validate_predictive_with_resolver(
    ...
    let avg_confidence = if valid_outputs > 0 {
        total_confidence / valid_outputs as f32
    } else {
        0.3
    };
...
fn build_intent_centroids(
    ...
    for example in examples {
        ...
        let signature = ClassificationSignature::compute_with_config(
            &example.question,
            &hasher,
...
fn optimize_feature_weights(
    ...
    let mut best_accuracy = evaluate_classification_accuracy(
        val_examples,
        intent_centroids,
        &best_weights,
        &hasher,
    ...
    for w_intent in [0.25, 0.30, 0.35, 0.40, 0.45] {
        for w_struct in [0.05, 0.08, 0.10, 0.12, 0.15] {
            for w_semantic in [0.10, 0.15, 0.20] {
                ...
                let accuracy = evaluate_classification_accuracy(
                    val_examples,
                    intent_centroids,
                    &weights,
                    &hasher,
    ...
    for _ in 0..50 {
        ...
        let weights = FeatureWeights {
            w_structure: (best_weights.w_structure + rng.gen_range(-perturbation..perturbation))
                .clamp(0.01, 0.30),
            w_punctuation: (best_weights.w_punctuation
                + rng.gen_range(-perturbation..perturbation))
            .clamp(0.01, 0.20),
            w_semantic: (best_weights.w_semantic + rng.gen_range(-perturbation..perturbation))
                .clamp(0.05, 0.30),
            w_derived: (best_weights.w_derived + rng.gen_range(-perturbation..perturbation))
                .clamp(0.01, 0.20),
            w_intent_hash: (best_weights.w_intent_hash
                + rng.gen_range(-perturbation..perturbation))
            .clamp(0.20, 0.50),
            w_tone_hash: (best_weights.w_tone_hash + rng.gen_range(-perturbation..perturbation))
                .clamp(0.05, 0.30),
        };
...
fn weighted_cosine_similarity(a: &[f32], b: &[f32], weights: &FeatureWeights) -> f32 {
    ...
    // Feature vector layout (82 elements total):
    // [0-13]:  base structural / punctuation / centroid / derived features
    // [14-45]: intent hash (32 bins)
    // [46-77]: tone hash (32 bins)
    // [78-81]: semantic probe flags

    let len = a.len().min(82);

    // Compute weighted similarity for each component
    let struct_sim = if len >= 3 {
        cosine_sim(&a[..3], &b[..3])
    } else {
        0.0
    };
    let punct_sim = if len >= 6 {
        cosine_sim(&a[3..6], &b[3..6])
    } else {
        0.0
    };
    let semantic_sim = if len >= 9 {
        cosine_sim(&a[6..9], &b[6..9])
    } else {
        0.0
    };
    let derived_sim = if len >= 14 {
        cosine_sim(&a[9..14], &b[9..14])
    } else {
        0.0
    };
    let intent_sim = if len >= 46 {
        cosine_sim(&a[14..46], &b[14..46])
    } else {
        0.0
    };
    let tone_sim = if len >= 78 {
        cosine_sim(&a[46..78], &b[46..78])
    } else {
        0.0
    };
    let semantic_flag_sim = if len >= 82 {
        cosine_sim(&a[78..82], &b[78..82])
    } else {
        0.0
    };
    ...
    weights.w_structure * struct_sim
        ...
        + weights.w_semantic * semantic_flag_sim
}
...
fn calibrate_confidence(
    ...
    let bands: Vec<(f32, f32)> = band_stats
        .iter()
        .map(|(threshold, correct, total)| {
            ...
            let calibration = if actual_accuracy > 0.01 {
                mid_confidence / actual_accuracy
            } else {
                1.0
            };
...
fn store_classification_model(
    ...
    for ic in intent_centroids {
        let _ =
            db.save_intent_centroid(&format!("{:?}", ic.intent), &ic.centroid, ic.example_count);
    }
    // Save tone centroids
    for tc in tone_centroids {
    ...
    for example in &examples_with_reasoning {
        if let Some(trace) = &example.reasoning {
            ...
            let confidences: Vec<f32> = if trace.confidence_trajectory.len() >= step_count {
                ...
                (0..step_count)
                    .map(|i| 0.5 + 0.4 * (i as f32 / step_count as f32))
                    .collect()
            };

            // Score each step using the weights
            let step_scores: Vec<f32> = trace
                .steps
                .iter()
                .enumerate()
                .map(|(idx, step)| {
                    ...
                    let context_score =
                        if step.content.len() > 50 { 0.8 } else { 0.5 } * weights.context;
                    let sequence_score = (idx as f32 / step_count as f32) * weights.sequence;
                    let transition_score = if step.dependencies.is_empty() {
                        0.5
                    } else {
                        0.8
                    } * weights.transition;
                    ...
                    let evidence_score =
                        if step.anchor_step { 0.9 } else { 0.5 } * weights.evidence;
            ...
            for i in 1..step_scores.len() {
                if step_scores[i] >= step_scores[i - 1] * 0.9 {
                    // Allow 10% tolerance
                    correctly_ordered += 1;
                }
            }
...
fn form_edges_from_qa(
    ...
    for (idx, example) in examples.iter().enumerate() {
        ...
        // L1-normalized edge formation preserves compound nouns before graph creation.
        let words = normalized_word_sequence(&example.answer, config);

        if words.len() < 2 {
            continue;
        }

        let base_edge_weight = config.builder.initial_utility_score;

        // Create edges between consecutive words
        for window in words.windows(2) {
            edge_batch.push((window[0].clone(), window[1].clone(), base_edge_weight));
            edges_created += 1;
        }

        let question_words = normalized_word_sequence(&example.question, config);
        let shared_question_words: Vec<String> = question_words
            .into_iter()
            .filter(|word| words.contains(word))
            .collect();

        for shared_word in shared_question_words {
            for answer_word in words.iter().take(3) {
                if shared_word != *answer_word {
                    edge_batch.push((shared_word.clone(), answer_word.clone(), base_edge_weight));
                    edges_created += 1;
                }
            }
        }
...
fn refine_layout(memory: &Arc<Mutex<MemoryStore>>, config: &EngineConfig) -> Result<(), String> {
    use crate::spatial_index::force_directed_layout;

    let layout = {
        let mem = memory.lock().map_err(|e| format!("Lock error: {}", e))?;
        force_directed_layout(&mem.all_units(), &config.semantic_map)
    };

    let mut mem = memory.lock().map_err(|e| format!("Lock error: {}", e))?;
    mem.update_positions(&layout.position_updates);
    Ok(())
}

fn detect_highways(memory: &Arc<Mutex<MemoryStore>>, config: &EngineConfig) -> Result<u64, String> {
    let mem = memory.lock().map_err(|e| format!("Lock error: {}", e))?;
    let db = mem.db();
    let threshold = config.semantic_map.highway_formation_threshold as u64;
    let max_length = config.semantic_map.highway_max_length.max(3);
    let mut highways_created = 0u64;

    let mut seed_words: Vec<(String, u64)> = mem
        .all_units()
        .into_iter()
        .filter(|unit| matches!(unit.level, crate::types::UnitLevel::Word))
        .map(|unit| (unit.normalized.clone(), unit.frequency))
        .collect::<Vec<_>>()
        .into_iter()
        .collect::<HashMap<_, _>>()
        .into_iter()
        .collect::<Vec<_>>();
    seed_words.sort_by(|lhs, rhs| rhs.1.cmp(&lhs.1));

    for (seed, _) in seed_words.into_iter().take(64) {
        if let Ok(edges) = db.load_word_edges(&seed) {
            for (target, _weight, traversal_count) in edges {
                if traversal_count >= threshold {
                    let mut sequence = vec![seed.to_string(), target.clone()];
                    let mut current = target;
                    for _ in 0..max_length.saturating_sub(2) {
                        if let Ok(next_edges) = db.load_word_edges(&current) {
                            if let Some((next_word, _, _next_count)) = next_edges
                                .into_iter()
                    ...
                    if sequence.len() >= 3
                        && traversal_count >= config.semantic_map.highway_min_frequency as u64
                    {
...
fn optimize_walk_parameters(
    ...
    let _ = val_examples;
    let _ = (
        config.semantic_map.tier1_confidence_threshold,
        config.semantic_map.tier2_confidence_threshold,
        config.semantic_map.pathfind_max_hops,
        config.semantic_map.pathfind_max_explored_nodes,
    );
    Ok(())
}

fn normalized_word_sequence(text: &str, config: &EngineConfig) -> Vec<String> {
    input::normalize_text_with_config(text, &config.classification)
        .split_whitespace()
        .map(|word| {
            word.chars()
