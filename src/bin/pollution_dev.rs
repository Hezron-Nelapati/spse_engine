...
use pollution_dev_lib::{
    config_sweep, generate_test_content, load_large_corpus, pollution_report, PollutionConfig,
    PollutionResult,
};
...
fn run_config_sweep() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "This will test multiple config combinations to find optimal anti-pollution settings.\n"
    );
    ...
    sorted.sort_by(|a, b| {
        a.1.pollution_score
            .partial_cmp(&b.1.pollution_score)
            .unwrap()
    });
...
fn print_pollution_summary(result: &PollutionResult) {
    ...
    println!(
        ...
        result.config_summary.min_frequency
    );
    println!(
        "  rolling_hash_window_sizes: {:?}",
        result.config_summary.window_sizes
    );
    println!(
        "  min_fragment_length: {}",
        result.config_summary.min_fragment_length
    );
    println!(
        "  punctuation_ratio_limit: {}",
        result.config_summary.punctuation_ratio
    );
}
