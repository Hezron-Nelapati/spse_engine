...
impl ReasoningDatasetGenerator {
    ...
    fn generate_single_hop_qa(&self, count: usize) -> Vec<TrainingExample> {
        ...
        let templates = vec![
            ...
            ("What is the capital of {country}?", "{capital} is the capital of {country}.", vec![
                "France:Paris", "Germany:Berlin", "Japan:Tokyo", "Brazil:Brasília", "Australia:Canberra",
                "Canada:Ottawa", "Mexico:Mexico City", "Spain:Madrid", "Italy:Rome", "China:Beijing",
                "India:New Delhi", "Russia:Moscow", "Egypt:Cairo", "Argentina:Buenos Aires", "Poland:Warsaw",
                "Sweden:Stockholm", "Norway:Oslo", "Denmark:Copenhagen", "Finland:Helsinki", "Netherlands:Amsterdam",
                "Belgium:Brussels", "Switzerland:Bern", "Austria:Vienna", "Portugal:Lisbon", "Greece:Athens",
                "Turkey:Ankara", "Iran:Tehran", "Iraq:Baghdad", "Israel:Jerusalem", "Thailand:Bangkok",
                "Vietnam:Hanoi", "Indonesia:Jakarta", "Philippines:Manila", "South Korea:Seoul", "Pakistan:Islamabad",
                "Bangladesh:Dhaka", "Nigeria:Abuja", "Kenya:Nairobi", "South Africa:Pretoria", "Morocco:Rabat",
                "Chile:Santiago", "Peru:Lima", "Colombia:Bogotá", "Venezuela:Caracas", "Ukraine:Kyiv",
                "Czech Republic:Prague", "Hungary:Budapest", "Romania:Bucharest", "Ireland:Dublin", "New Zealand:Wellington",
            ]),
            // Population questions (30+ variants)
            ("What is the population of {city}?", "The population of {city} is approximately {population}.", vec![
                "Paris:2.1 million", "Berlin:3.6 million", "Tokyo:14 million", "New York:8.3 million",
                "London:9 million", "Mumbai:20 million", "Shanghai:27 million", "São Paulo:12 million",
                "Moscow:12 million", "Cairo:10 million", "Lagos:15 million", "Istanbul:15 million",
                "Beijing:21 million", "Delhi:32 million", "Los Angeles:4 million", "Chicago:2.7 million",
                "Toronto:3 million", "Sydney:5 million", "Melbourne:5 million", "Singapore:5.7 million",
                "Hong Kong:7.5 million", "Bangkok:10 million", "Seoul:10 million", "Manila:14 million",
                "Jakarta:11 million", "Karachi:15 million", "Dhaka:22 million", "Lima:10 million", "Bogotá:8 million",
            ]),
            // Birth years (40+ variants)
            ("When was {person} born?", "{person} was born in {year}.", vec![
                "Albert Einstein:1879", "Marie Curie:1867", "Isaac Newton:1643", "Galileo Galilei:1564",
                "Charles Darwin:1809", "Nikola Tesla:1856", "Thomas Edison:1847", "Leonardo da Vinci:1452",
                "William Shakespeare:1564", "Wolfgang Mozart:1756", "Ludwig Beethoven:1770", "Johann Bach:1685",
                "Abraham Lincoln:1809", "George Washington:1732", "Napoleon Bonaparte:1769", "Mahatma Gandhi:1869",
                "Martin Luther King:1929", "Nelson Mandela:1918", "Winston Churchill:1874", "Franklin Roosevelt:1882",
                "Ada Lovelace:1815", "Alan Turing:1912", "Grace Hopper:1906", "Richard Feynman:1918",
                "Stephen Hawking:1942", "Carl Sagan:1934", "Jane Goodall:1934", "Rachel Carson:1907",
                "Sigmund Freud:1856", "Carl Jung:1875", "Karl Marx:1818", "Friedrich Nietzsche:1844",
                "Plato:428 BC", "Aristotle:384 BC", "Socrates:470 BC", "Confucius:551 BC",
                "Cleopatra:69 BC", "Julius Caesar:100 BC", "Alexander the Great:356 BC", "Genghis Khan:1162",
            ]),
            // Definitional questions (30+ variants)
            ("What is {term}?", "{term} is {definition}.", vec![
            ...
            ("What are the main components of {system}?", "The main components of {system} are {components}.", vec![
        ...
        for (q_template, a_template, variants) in templates {
            for variant in variants.iter().cycle().take(per_template) {
                ...
                let question = q_template.replace("{country}", parts[0]).replace("{city}", parts[0]).replace("{person}", parts[0]).replace("{term}", parts[0]).replace("{concept}", parts[0]).replace("{system}", parts[0]);
                let answer = a_template.replace("{capital}", parts[1]).replace("{population}", parts[1]).replace("{year}", parts[1]).replace("{definition}", parts[1]).replace("{components}", parts[1]).replace("{country}", parts[0]).replace("{city}", parts[0]).replace("{person}", parts[0]).replace("{concept}", parts[0]).replace("{system}", parts[0]);
                
                let reasoning = Some(ReasoningTrace {
                    steps: vec![
                        ReasoningStep {
                            step_type: ReasoningStepType::Premise,
                            content: format!("Query asks for factual information about {}", parts[0]),
                            anchor_step: false,
                            dependencies: vec![],
                            structure_hash: None,
                        },
    ...
    fn generate_multi_hop_qa(&self, count: usize) -> Vec<TrainingExample> {
        ...
        let comparison_variants = vec![
            ("Paris", "Berlin", "Berlin (3.6M) is larger than Paris (2.1M)."),
            ("Tokyo", "London", "Tokyo (14M) is larger than London (9M)."),
            ("Sydney", "Melbourne", "Melbourne (5M) is slightly larger than Sydney (4.6M)."),
        ];
        ...
        let analysis_variants = vec![
            ...
            ("digestion", "Digestion breaks down food into nutrients through enzymes and acids."),
            ("respiration", "Respiration converts glucose and oxygen into energy, water, and carbon dioxide."),
        ];
    ...
    fn generate_adversarial_qa(&self, count: usize) -> Vec<TrainingExample> {
        ...
        let adversarial_templates = vec![
            ("What is the population of {fictitious_city}?", "I don't have reliable information about the population of {fictitious_city}.", vec!["Atlantis", "El Dorado", "Shangri-La"]),
            ("When was {impossible_event} invented?", "The premise of the question is flawed - {impossible_event} is not an invention.", vec!["time travel", "perpetual motion", "teleportation"]),
        ];
        ...
        for (q_template, a_template, variants) in adversarial_templates {
            for variant in variants.iter().cycle().take(per_template) {
                let question = q_template.replace("{fictitious_city}", variant).replace("{impossible_event}", variant);
                let answer = a_template.replace("{fictitious_city}", variant).replace("{impossible_event}", variant);
...
mod tests {
    ...
    #[test]
    fn generates_reasoning_dataset() {
        let generator = ReasoningDatasetGenerator::new();
        let examples = generator.generate_full_dataset();
        
        assert!(examples.len() >= 20000, "Should generate 20K+ examples");
        
        // Verify distribution
        let single_hop = examples.iter().filter(|e| e.curriculum.difficulty == "easy").count();
        let multi_hop = examples.iter().filter(|e| e.curriculum.difficulty == "medium").count();
        let adversarial = examples.iter().filter(|e| e.curriculum.difficulty == "hard").count();
        
        assert!(single_hop > multi_hop, "Single-hop should be majority");
        assert!(multi_hop > adversarial, "Multi-hop should exceed adversarial");
    }
}
