...
mod tests {
    ...
    use crate::types::{
        MemoryChannel, MemoryType, MetadataSummary, RetrievedDocument, ScoreBreakdown,
    };
    use chrono::Utc;
    use uuid::Uuid;

    fn scored(score: f32) -> ScoredCandidate {
        ScoredCandidate {
            unit_id: Uuid::new_v4(),
    ...
    fn document_with_metadata(
        ...
        RetrievedDocument {
            ...
            normalized_content: title.to_lowercase(),
    ...
    fn unit() -> Unit {
        Unit {
            ...
            normalized: "paris".to_string(),
            level: crate::types::UnitLevel::Word,
            frequency: 1,
            utility_score: 0.8,
            ...
            content_lower: "paris".to_string(),
            content_fingerprint: crate::types::text_fingerprint("paris"),
        }
    }

    #[test]
    fn micro_validator_skips_clear_high_trust_winner() {
        let docs = vec![document_with_metadata(
            "Paris",
            0.8,
            MetadataSummary {
                numbers: vec![("paris".to_string(), 2.1, "million".to_string())],
                ..MetadataSummary::default()
            },
        )];

        assert!(!should_run_micro_validator(
            &[scored(0.90), scored(0.70)],
            &docs,
            &EvidenceMergeConfig::default().micro_validator,
        ));
    }

    #[test]
    fn micro_validator_penalizes_contradictory_numbers() {
        let docs = vec![
            ...
            document_with_metadata(
                ...
                0.55,
                MetadataSummary {
                    numbers: vec![("paris".to_string(), 3.4, "million".to_string())],
                    ..MetadataSummary::default()
                },
            ),
        ];

        let merged = EvidenceMerger.merge(
            &[unit()],
            ...
            &[scored(0.80), scored(0.79)],
            &EvidenceMergeConfig::default(),
        );

        assert!(!merged.conflict_records.is_empty());
        assert!(merged.evidence_support < 0.575 * 0.50 + 0.20);
    }
}
