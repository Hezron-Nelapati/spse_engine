        ...
        for iteration in 0..config.iterations {
            ...
            if simulation_result.recovered {
                ...
                result.failures.push(format!(
                    ...
                    crash_point,
                    iteration,
                    simulation_result.error.unwrap_or_default()
                ));
            }
...
pub fn simulate_crash(crash_point: CrashPoint, iteration: usize) -> CrashSimulationResult {
    ...
    let crash_result = match crash_point {
        CrashPoint::ForceDirectedMidIteration => simulate_force_directed_crash(&engine),
        CrashPoint::SpatialGridRebuild => simulate_spatial_grid_crash(&engine),
        CrashPoint::LfuPruning => simulate_lfu_pruning_crash(&engine),
        CrashPoint::CandidatePromotion => simulate_candidate_promotion_crash(&engine),
    };
