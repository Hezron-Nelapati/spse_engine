...
pub mod categories {
    ...
    pub static CASUAL_KEYWORDS: Lazy<[&str; 14]> = Lazy::new(|| {
        [
            ...
            "gonna", "wanna", "kinda", "sorta", "lol", "haha", "ok",
        ]
    });
    ...
    pub static MATH_PATTERNS: Lazy<[&str; 6]> = Lazy::new(|| {
        ["=", "+", "-", "*", "/", " is "]
    });
}
...
mod tests {
    ...
    fn test_contains_any() {
        assert!(KeywordMatcher::contains_any("hello world", &["hello", "foo"]));
        assert!(!KeywordMatcher::contains_any("hello world", &["bar", "foo"]));
    }

    #[test]
    fn test_count_matches() {
        assert_eq!(KeywordMatcher::count_matches("urgent now", &["urgent", "now", "foo"]), 2);
    }

    #[test]
    fn test_score_matches() {
        let score = KeywordMatcher::score_matches("urgent critical", &["urgent", "critical"], 0.15, 1.0);
