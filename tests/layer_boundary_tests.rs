...
fn l1_to_l2_input_packet_schema() {
    ...
    if has_activated || has_new {
        ...
        for unit in &output.activated_units {
            ...
            assert!(
                ...
                "Unit normalized text must not be empty"
            );
            assert!(
                unit.utility_score >= 0.0 && unit.utility_score <= 1.0,
                "Unit utility score must be in [0, 1] range, got {}",
                unit.utility_score
            );
            assert!(
                unit.confidence >= 0.0 && unit.confidence <= 1.0,
                "Unit confidence must be in [0, 1] range, got {}",
                unit.confidence
            );
            assert!(
                unit.frequency >= 1,
                "Unit frequency must be at least 1 for new units"
            );
        }
        ...
        for unit in &output.new_units {
            assert!(
                ...
                "New unit content must not be empty"
            );
            assert!(
                !unit.normalized.is_empty(),
                "New unit normalized text must not be empty"
            );
        }
    }
    ...
    assert!(
        has_activated || has_new || output.activated_units.is_empty(),
...
fn l2_to_l3_unit_stream() {
    ...
    if input_unit_count > 0 {
        assert!(
            ...
            "L3 must produce non-empty levels map from L2 units (input had {} units)",
            input_unit_count
        );
    }
    ...
    for (level, units) in &hierarchy.levels {
        ...
        for unit in units {
            assert!(
                !unit.content.is_empty(),
                "Unit in level '{}' has empty content",
                level
            );
            assert!(
                unit.utility_score >= 0.0,
                "Unit in level '{}' has invalid utility",
                level
            );
        }
...
fn l3_to_l4_hierarchy_memory() {
    ...
    for id in &active_ids {
        ...
        assert!(
            unit.is_some(),
            "Active unit {} must be retrievable from store",
            id
        );
        ...
        assert!(
            !unit.content.is_empty(),
            "Unit {} must have non-empty content",
            id
        );
        assert_eq!(
            unit.memory_type,
            MemoryType::Episodic,
            "Unit {} must have correct memory type",
            id
        );
    }
...
fn l5_to_l6_spatial_context() {
    ...
    if expected_count > 0 {
        assert!(
            unit_count > 0,
            "L5 must provide at least one unit for L6 context"
        );
        assert!(
            unit_count <= expected_count,
            "Unit count ({}) cannot exceed expected ({})",
            unit_count,
            expected_count
        );
    }
    ...
    assert!(
        context.cells.is_empty() || !context.cells.is_empty(),
        "ContextMatrix must have valid cells field"
    );
    assert!(
        context.summary.is_empty() || !context.summary.is_empty(),
...
fn l6_to_l7_context_intent() {
    ...
    assert!(
        profile.confidence >= 0.0 && profile.confidence <= 1.0,
        "L7 must produce confidence in [0, 1] range, got {}",
        profile.confidence
    );
...
fn l7_to_l9_intent_retrieval() {
    ...
    assert!(
        low_confidence_triggers_retrieval || !low_confidence_triggers_retrieval,
        "L9 must evaluate confidence-based retrieval trigger"
    );
    assert!(
        question_intent_may_retrieve || !question_intent_may_retrieve,
    ...
    assert!(
        ...
        "L7 confidence must be non-negative for L9"
    );
    assert!(
...
fn l14_to_l16_candidate_resolver() {
    ...
    use spse_engine::types::{ResolverMode, ScoreBreakdown, ScoredCandidate};
...
fn l16_to_l17_resolver_output() {
    ...
    assert!(
        output.grounded || !output.grounded,
...
fn state_consistency_unit_id() {
    ...
    if !active_ids.is_empty() {
        for id in &active_ids {
            ...
            assert!(
                unit.is_some(),
                "Unit ID {} must be retrievable from store",
                id
            );
            let unit = unit.unwrap();
        ...
        for id in &active_ids {
            assert!(
                seen.insert(*id),
                "Duplicate unit ID {} found in active_ids",
                id
            );
        }
    }
...
fn state_consistency_context_overflow() {
    ...
    if total_ingested > 0 && stored_count > 0 {
        assert!(
            stored_count >= total_ingested || stored_count < total_ingested + 100,
...
fn state_consistency_unit_dropped() {
    ...
    if !all_ids.is_empty() {
        assert!(
            stored_count > 0,
            "Memory must contain at least some units from quality inputs"
        );
        ...
        for id in &all_ids {
            ...
            assert!(
                unit.content.len() > 1,
                "Stored unit {} must have meaningful content (len > 1)",
                id
            );
        }
...
fn state_consistency_high_throughput() {
    ...
    if total_units > 0 {
        assert!(
            stored_count > 0,
            "Memory must store units after high throughput"
        );
        assert!(
            stored_count <= total_units,
            "Stored count ({}) cannot exceed processed count ({})",
            stored_count,
            total_units
        );
    }
    ...
    assert!(
        duration.as_secs() < 30,
        "High throughput test took too long: {:?} for {} documents",
        duration,
        doc_count
    );
...
fn backpressure_retrieval_bottleneck() {
    ...
    if total_processed > 0 {
        assert!(
            stored_count > 0,
            "Memory must contain units after bottleneck handling"
        );
        assert!(
            stored_count <= total_processed,
            "Stored count ({}) cannot exceed processed ({})",
            stored_count,
            total_processed
        );
    }
...
fn backpressure_queue_overflow() {
    ...
    if total_processed > 0 {
        assert!(
            stored_count > 0,
            "Memory must contain units after overflow handling"
        );
...
fn backpressure_sustained_high_rate() {
    ...
    if total_processed > 0 {
        assert!(
            stored_count > 0,
            "Memory must contain units after sustained processing"
        );
        assert!(
            stored_count <= total_processed,
            "Stored count ({}) cannot exceed processed ({})",
    ...
    assert!(
        duration.as_secs() < 60,
        "Sustained test took too long: {:?} for {} documents",
        duration,
        doc_count
    );
