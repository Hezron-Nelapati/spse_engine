...
fn conf_formula_lower_bound_when_zero_best() {
    ...
    assert_eq!(
        c, 0.0,
        "ISSUE-1: zero best_sim must return 0.0 (spec missing guard)"
    );
}
...
fn conf_formula_margin_blend_extremes() {
    ...
    assert!(
        (c1 - 0.80).abs() < EPSILON,
        "margin_blend=1.0 must equal best_sim"
    );
    ...
    assert!(
        (c2 - 0.80).abs() < EPSILON,
        "margin_blend=0.0, runner_up=0 must equal best_sim"
    );
}
...
fn reasoning_loop_triggers_retrieval_for_very_low_confidence() {
    ...
    assert!(
        retrieval,
        "retrieval must trigger for very low initial confidence"
    );
...
fn reasoning_loop_not_triggered_above_floor() {
    ...
    assert_eq!(
        traj.len(),
        1,
        "no loop steps for confidence above trigger floor"
    );
}
...
fn edge_weight_bounded_zero_to_one() {
    for (base, freq, decay, epochs) in &[(0.9, 100, 0.01, 0), (0.1, 1, 0.5, 100), (1.0, 50, 0.0, 0)]
    {
...
fn tier_selection_falls_to_tier2_for_weak_near_edge() {
    ...
    assert_eq!(
        tier,
        Tier::Tier2Far,
        "strong far edge must select Tier 2 when near is weak"
    );
}
...
fn tier_selection_falls_to_tier3_for_no_good_edges() {
    ...
    assert_eq!(
        tier,
        Tier::Tier3Pathfind,
        "low-score edges must fall to Tier 3"
    );
}
...
fn tier_selection_tier3_when_no_edges_at_all() {
    ...
    assert_eq!(
        tier,
        Tier::Tier3Pathfind,
        "no edges at all must select Tier 3"
    );
}
...
fn beam_width_one_is_greedy() {
    ...
    assert_eq!(
        greedy_path,
        vec![0, 1, 3],
        "beam_width=1 must follow greedy path 0→1→3"
    );
}
...
fn beam_search_log_score_no_underflow() {
    ...
    assert!(
        !path.is_empty(),
        "beam search must not underflow on long walks"
    );
...
fn highway_forms_at_threshold_traversals() {
    ...
    for &count in &traversal_counts {
        ...
        if count < 5 {
            ...
            assert!(
                should_form,
                "count {} must form highway (threshold=5)",
                count
            );
        }
    }
...
fn ttg_edge_graduates_after_second_traversal() {
    ...
    let edge = EdgeStatus::Probationary {
        traversal_count: 1,
    ...
    assert_eq!(
        next,
        EdgeStatus::Episodic,
        "TTG: 2 traversals must graduate edge to Episodic"
    );
}
...
fn ttg_edge_promoted_on_lease_expiry_if_count_sufficient() {
    ...
    assert_eq!(
        next,
        EdgeStatus::Episodic,
        "TTG: late graduation when lease expired + count sufficient"
    );
}
...
fn temperature_zero_selects_argmax() {
    ...
    assert!(
        (probs[1] - 1.0).abs() < EPSILON,
        "temperature=0 must select argmax (index 1)"
    );
...
fn temperature_sampling_probabilities_sum_to_one() {
    ...
    for temp in &[0.0f32, 0.1, 0.5, 0.9] {
        ...
        assert!(
            (sum - 1.0).abs() < 1e-5,
            "probabilities must sum to 1.0 at temperature={}",
            temp
        );
    }
}
...
fn pos_weighted_position(
    base: [f32; 3],
    pos_tag: &str,
    context_hash: Option<[f32; 3]>,
) -> [f32; 3] {
...
fn polysemous_words_get_different_initial_positions() {
    ...
    assert!(
        dist > 0.01,
        "polysemous words must have different initial positions (dist={:.4})",
        dist
    );
}
...
fn position_offset_bounded_by_spec_constants() {
    ...
    assert!(
        dist <= 0.25,
        "initial position shift must be small (≤0.25), got {:.4}",
        dist
    );
}
...
fn is_social_intent(intent: IntentKind) -> bool {
    matches!(
        intent,
        IntentKind::Greeting | IntentKind::Farewell | IntentKind::Gratitude
    )
}
...
fn is_factual_intent(intent: IntentKind) -> bool {
    matches!(
        intent,
        IntentKind::Verify | IntentKind::Question | IntentKind::Explain
    )
}
...
fn r7_broad_sparsity_detected_across_multiple_intents() {
    ...
    assert_eq!(
        loss.r7, 1,
        "R7 broad sparsity not detected across 3 intents"
    );
}
...
fn r7_not_triggered_when_fewer_than_threshold_intents_sparse() {
    ...
    assert_eq!(
        loss.r7, 0,
        "R7 must NOT trigger for only 2 sparse intents (threshold=3)"
    );
}
...
fn hysteresis_split_threshold_is_0_40() {
    assert_eq!(
        structural_feedback_action(0.41, 0.40, 0.20),
        StructuralAction::Split
    );
    assert_eq!(
        structural_feedback_action(0.40, 0.40, 0.20),
        StructuralAction::NoAction
    );
}
...
fn hysteresis_merge_threshold_is_0_20() {
    assert_eq!(
        structural_feedback_action(0.19, 0.40, 0.20),
        StructuralAction::Merge
    );
    assert_eq!(
        structural_feedback_action(0.20, 0.40, 0.20),
        StructuralAction::NoAction
    );
}
...
fn numerical_stability_suite() {
    ...
    for (best, runner) in &[(0.0, 0.0), (1.0, 0.0), (0.5, 0.5), (0.5, 0.0)] {
        ...
        assert!(
            !c.is_nan(),
            "confidence must not be NaN for best={}, runner={}",
            best,
            runner
        );
    ...
    assert!(
        ...
        "300-step log-score must not underflow"
    );
}
...
fn extract_structural_features(text: &str) -> Vec<f32> {
    ...
    let has_wh_word = if words.iter().any(|w| wh_words.contains(w)) {
        1.0
    } else {
        0.0
    };
    ...
    let starts_with_imperative = if words
        .first()
        .map(|w| imperative_starters.contains(w))
        .unwrap_or(false)
    {
        1.0
    } else {
    ...
    let starts_with_social = if words
        .first()
        .map(|w| social_words.contains(w))
        .unwrap_or(false)
    ...
    let has_comparison = if words.iter().any(|w| comparison_cues.contains(w)) {
        1.0
    } else {
        0.0
    };
    ...
    let has_negation = if words.iter().any(|w| negations.contains(w)) {
        1.0
    } else {
        0.0
    };
    ...
    let has_tech = if words.iter().any(|w| tech_cues.contains(w)) {
        1.0
    } else {
        0.0
    };
    ...
    let has_creative =
        if words.iter().any(|w| creative_cues.contains(w)) || lower.contains("write a") {
            1.0
        } else {
            0.0
        };
    ...
    let has_factual = if words.iter().any(|w| factual_anchors.contains(w)) {
        1.0
    } else {
        0.0
    };
    ...
    let conjunctions = [
        "and", "but", "or", "nor", "for", "so", "yet", "although", "because", "while", "since",
        "unless", "if", "when",
    ];
...
fn rw_classify_capital_of_france_is_question() {
    ...
    assert!(
        conf >= 0.40,
        "factual question confidence must be ≥0.40 (got {:.3} mode={})",
        conf,
        mode
    );
}
...
fn rw_classify_who_invented_telephone_is_question() {
    ...
    assert_eq!(
        intent, "Question",
        "\"Who invented the telephone?\" must classify as Question"
    );
}
...
fn rw_reasoning_loop_entry_matches_expected_for_all_queries() {
    ...
    for tc in reasoning_test_cases() {
        let (traj, retrieval) =
            reasoning_loop_trajectory(tc.initial_conf, tc.max_steps, TRIGGER_FLOOR, EXIT_THRESHOLD);
        ...
        for &c in &traj {
            assert!(
                c >= 0.0 && c <= 1.0 + EPSILON,
                "Query {:?}: confidence out of bounds: {:.4}",
                tc.query,
                c
            );
        }
...
fn rw_social_queries_never_enter_reasoning_loop() {
    ...
    for (query, init_conf) in &social_queries {
        let (traj, _retrieval) =
            reasoning_loop_trajectory(*init_conf, 3, TRIGGER_FLOOR, EXIT_THRESHOLD);
        assert_eq!(
            traj.len(),
            1,
...
fn rw_ambiguous_queries_enter_reasoning_loop_and_improve() {
    ...
    for (query, init_conf) in &ambiguous {
        let (traj, _retrieval) =
            reasoning_loop_trajectory(*init_conf, 5, TRIGGER_FLOOR, EXIT_THRESHOLD);
        assert!(
            traj.len() > 1,
            "Ambiguous query {:?} (conf={:.2}) must enter reasoning loop",
            query,
            init_conf
        );
        ...
        assert!(
            ...
            "Reasoning loop must improve confidence for {:?}",
            query
        );
    }
}
...
fn build_word_graph_from_sentences(
    sentences: &[&str],
) -> (HashMap<String, usize>, HashMap<usize, Vec<(usize, f32)>>) {
...
fn rw_beam_search_continues_what_is_the() {
    ...
    assert!(
        path.len() >= 2,
        "beam search must produce at least 2 words from \"what\""
    );
    assert_eq!(
        path[1], is_id,
        "second word after \"what\" must be \"is\" (id={}), got id={}",
        is_id, path[1]
    );
    assert_eq!(
        path[2], the_id,
        "third word must be \"the\" (id={}), got id={}",
        the_id, path[2]
    );
    ...
    if path.len() >= 4 {
        assert_eq!(
            path[3], capital_id,
            "fourth word after \"the\" must be \"capital\" (most frequent), got id={}",
            path[3]
        );
    }
}
...
fn rw_beam_search_prefers_high_frequency_continuation() {
    ...
    assert_eq!(
        path[1], capital_id,
        "greedy step from \"the\" must pick \"capital\" (3× seen), got id={}",
        path[1]
    );
}
...
fn rw_word_graph_highway_forms_for_frequent_pair() {
    ...
    for &count in &traversals_before_highway {
        ...
        if count < 5 {
            assert!(
                !is_highway,
                "\"of france\" traversal count {} must not be a highway yet",
                count
            );
        } else {
            assert!(
                is_highway,
                "\"of france\" traversal count {} must form highway",
                count
            );
        }
    }
...
fn rw_real_sentence_beam_produces_coherent_word_sequence() {
    ...
    for &id in &path {
        assert!(
            ...
            "path contains unknown word ID {} (not in vocabulary)",
            id
        );
    }
    ...
    assert!(
        path.len() >= 3,
        "real-world sentence walk must produce ≥3 words, got {}",
        path.len()
    );
}
...
fn assert_answer_oracle(actual: &str, accepted_variants: &[&str], required_terms: &[&str]) {
    ...
    assert!(
        !canonical_actual.is_empty(),
        "pipeline answer must not be empty"
    );
    ...
    if !accepted_variants.is_empty() {
        ...
        assert!(
            ...
            actual, canonical_actual, accepted_variants
        );
    }
...
fn run_pipeline(
    ...
    let (traj, retrieval) =
        reasoning_loop_trajectory(reasoning_confidence, 3, TRIGGER_FLOOR, EXIT_THRESHOLD);
...
fn rw_pipeline_hello_is_coherent_end_to_end() {
    ...
    let result = run_pipeline(
        "Hello", "Greeting", None, &centroids, &vocab, &edges, "hello", 1, 4,
    );
...
fn rw_pipeline_capital_of_france_generates_answer_after_retrieval() {
    let centroids = build_intent_centroids();
    let (vocab, edges) = build_word_graph_from_sentences(&["paris is the capital of france"]);
...
fn rw_pipeline_translate_query_stays_local() {
    let centroids = build_intent_centroids();
    let (vocab, edges) = build_word_graph_from_sentences(&["buenos dias", "bonjour"]);
    ...
    assert_answer_oracle(&result.final_text, &["buenos dias"], &["buenos", "dias"]);
}

#[test]
fn rw_pipeline_plan_query_stays_local() {
    let centroids = build_intent_centroids();
    let (vocab, edges) = build_word_graph_from_sentences(&["define scope build test launch"]);
...
fn rw_pipeline_summarize_query_stays_local() {
    let centroids = build_intent_centroids();
    let (vocab, edges) = build_word_graph_from_sentences(&["main points evidence next steps"]);
...
fn rw_pipeline_math_query_generates_numeric_answer() {
    ...
    assert_answer_oracle(&result.final_text, &["four", "4"], &["four"]);
}
...
fn rw_pipeline_recommend_query_generates_actionable_answer() {
    let centroids = build_intent_centroids();
    let (vocab, edges) =
        build_word_graph_from_sentences(&["sony alpha is a good beginner travel camera"]);
...
fn rw_pipeline_consistency_rules_satisfied_for_all_canonical_inputs() {
    ...
    let canonical_cases: Vec<(&str, IntentKind, f32, bool, usize, bool, bool, bool, bool)> = vec![
        ...
        (
            "Could you sort of explain something?",
            IntentKind::Explain,
            0.34,
            true,
            2,
...
fn rw_issue_rw3_dead_band_queries_loop_without_retrieval() {
    ...
    for (query, init_conf) in dead_band_cases {
        ...
        assert!(
            traj.len() > 1,
            "Dead-band query {:?} (conf={:.2}) must enter loop",
            query,
            init_conf
        );
        assert!(
...
fn rw_issue_rw2_write_poem_and_explain_have_different_feature_vectors() {
    ...
    assert!(
        sim < 1.0 - EPSILON,
        "\"Write a poem\" and \"Explain how...\" must NOT be identical vectors (sim={:.4})",
        sim
    );
    assert!(
        sim > 0.3,
        "but they should share some structural similarity (imperative start), sim={:.4}",
        sim
    );
}
